...
int main(int argc, char **argv)
{
	...
	if (cfg.filename[0] != 0) {
		...
		//map_port = bpf_object__find_map_by_name(bpf_obj, "puerto_map");
		xsks_map_fd = bpf_map__fd(map);
		//map_port_fd = bpf_map__fd(map_port);
		printf("FILE DESCRIPTOR MAP XSKS PORT: %d\n",xsks_map_fd);
		//printf("FILE DESCRIPTOR MAP PORT: %d\n",map_port_fd);
		if (xsks_map_fd < 0) {
	...
	/*Cargo mapa puerto*/
	printf("Cargamos mapa puerto_map Puerto:%d y Key:%d\n", numero_puerto,key_puerto);
	int map_puerto_fd;
	map_puerto_fd = bpf_object__find_map_fd_by_name(bpf_obj,"xdp_puerto_map");

	ret = bpf_map_update_elem(map_puerto_fd, &key_puerto, &numero_puerto, BPF_ANY); //actualizar mapa de usuario
